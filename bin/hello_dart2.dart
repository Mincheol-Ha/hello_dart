...
void main(List<String> arguments) {
  ...
  // 변수 선언 3 - 동적타입 변수
  // name = 123; // 일단 타입이 추론되어 자료형이 결정되면 나중에 변경 불가!
  dynamic name3 = '홍길동'; // 처음엔 문자열 타입이었다가
  name3 = 123;             // 나중에는 숫자 타입으로 바꿔도 가능. (비추)
  ...
  print(scores.keys); // socres.values

  // null 연산자
  // ? : 변수에 null 허용여부를 명시적으로 선언 (Nullable 타입)
 // int score;     // null에 대한 명시적 선언 없음
 // print(score);   // 이런 경우, 변수 초기화없이 호출 분가!
  ...
  // ?. : null-aware 접근연산자
  // 객체가 null이면 null을 반황, 그렇지 않으면 해당 속성이나 메서드를 호출
  Person? person;

  person?.sayHello(); // person이 null 이므로 아무 작업도 수행되지 않음
